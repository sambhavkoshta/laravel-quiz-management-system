...
<form action="{{url('/mcqs/add/'.$mcq->id)}}" method="post">
    ...
    <select name="correct_answer">
        ...
        <option value="B" {{$mcq->correct_answer == 'A' ? 'selected' : ''}}>B</option>
        <option value="C" {{$mcq->correct_answer == 'A' ? 'selected' : ''}}>C</option>
        <option value="D" {{$mcq->correct_answer == 'A' ? 'selected' : ''}}>A</option>
    </select>
