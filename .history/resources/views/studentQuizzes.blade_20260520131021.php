<x-layout>
    <x-slot name="main">
        ...
        <div class="min-h-screen bg-gray-100 m-4 rounded-xl overflow-hidden">
            ...
            <div class=" m-4 rounded-xl shadow items-center p-4 bg-white max-w-md mx-auto space-x-2 text-xl text-gray-700 text-center">
                <span>{{$quiz->name}}</span>
                <span class="text-sm bg-blue-600 px-4 py-2 text-white rounded-xl"><a href="{{url('/quiz/start/'.$quiz->id)}}">Start Quiz</a></span>
                <span class="text-sm bg-blue-600 px-4 py-2 text-white rounded-xl"><a href="{{url('/quiz/leaderboard/'.$quiz->id)}}">Leaderboard</a></span>
            </div>
            @endforeach
            <hr>
