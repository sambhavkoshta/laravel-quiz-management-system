    ...
    <x-slot name="main">
        <div class="min-h-screen bg-gray-100 m-4 rounded-xl overflow-hidden">
            ...
            <div class=" px-4 py-2 text-xl bg-white max-w-sm  mx-auto space-x-4 rounded-2xl shadow-2xl m-6 text-gray-700 text-center">
                <span class="text-gray-800 text-xl">{{$category->name}}</span> <span class="text-sm bg-blue-600 px-2 py-1 text-white rounded-xl"><a href="{{url('/student/quizzes/'.$category->id)}}">View Quiz</a></span></div>
            @endforeach
            @else
