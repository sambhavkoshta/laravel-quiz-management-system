    ...
    <x-slot name="main">
        <div class="flex flex-col items-center justify-center min-h-screen p-4">
            <div class=" w-full max-w-md bg-white flex flex-col items-center p-8 rounded-2xl shadow-lg">
                <h1 class="text-3xl font-bold text-gray-700 mb-4">Admin Login</h1>
                <form action="{{url('/admin-login')}}" method="post" class="space-y-4 w-full m-4">
                    @csrf
                    ...
                    @error('username')
                    <div class="text-red-500 py-2">{{$message}}</div>
                    @enderror
