...
<x-layout>
    <x-slot name="main">
        <div class="min-h-screen bg-white m-4 rounded-xl">
            ...
            @foreach($quizzes as $quiz)
            <div class="p-4 text-xl text-gray-700 text-center">Quiz : {{$quiz->name}} <span class="text-sm bg-blue-600 px-2 py-1 text-white rounded-xl"><a href="{{url('/quiz/start/'.$quiz->id)}}">View Quiz</a></span></div>
            @endforeach
        </div>
    </x-slot>
</x-layout>