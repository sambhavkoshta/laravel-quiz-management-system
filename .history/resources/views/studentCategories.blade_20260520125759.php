<x-layout>
    <x-slot name="main">
        <div class="min-h-screen bg-gray-100 m-4 rounded-xl">
            ...
            <div class="p-4 text-xl bg-white text-gray-700 text-center">Category : {{$category->name}} <span class="text-sm bg-blue-600 px-2 py-1 text-white rounded-xl"><a href="{{url('/student/quizzes/'.$category->id)}}">View Quiz</a></span></div>
            @endforeach
            @else
