...
class StudentQuizController extends Controller
{
    ...
    function submitQuiz(Request $request, $id)
    {
        ...
        $score = 0;
        foreach ($mcqs as $mcq) {
            $selectedOptionId =$request->answers[$mcq->id] ?? null;
        ...
        return view('result',compact('score','quiz','mcqs','answers'));
    }
    ...
    function leaderBoard($id){
        $quiz = Quiz::findOrFail($id);
        $results = Result::with('student')->where('quiz_id', $id)->orderByDesc('score')->get();
        return view(
            'leaderboard',
            compact('results', 'quiz')
        );
    }
}
